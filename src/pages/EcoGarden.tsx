...
import { ArrowLeft, Leaf, Droplet, Sun, Trophy, Sprout, Shield, Zap, Earth } from "lucide-react";
...
export const EcoGarden = () => {
  ...
  // Plant health percentage - starting at 72% but can be changed
  const [plantHealth, setPlantHealth] = useState(72);
  
  // Plant stage images array
  const plantStages = [plantStage1, plantStage2, plantStage3, plantStage4, plantStage5];
  
  // Stage names and descriptions
  const stageInfo = [
    { name: "Seedling", level: "Level 1/5", description: "Just starting to grow" },
        ...
        <div className="w-full space-y-4">
          ...
          <div className="extension-card">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-lg font-medium text-gray-800">Your Eco Garden</h1>
            </div>
          </div>
          ...
          <div className="extension-card text-center space-y-3">
            <div className="relative">
              <img 
                ...
                style={{
                  filter: isAnimating ? 'drop-shadow(0 0 20px rgba(34, 197, 94, 0.5))' : 'none'
                }}
              />
              {isAnimating && (
            ...
            <div>
              <h2 className="text-lg font-medium text-gray-800">{stageInfo[currentStage - 1].name}</h2>
              <p className="text-sm text-gray-500">{stageInfo[currentStage - 1].level}</p>
              <p className="text-xs text-gray-400 mt-1">{stageInfo[currentStage - 1].description}</p>
            </div>
            <div className="space-y-2">
              <Progress value={plantHealth} className="h-2" />
              <p className="text-sm text-gray-600">
                Plant health: <span className="text-green-500 font-medium">{plantHealth}%</span>
              </p>
          ...
          <div className="extension-card space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-base font-medium text-gray-800">Plant Needs</h3>
            </div>
